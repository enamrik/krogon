...
def delete_gclb(k_mci: KubeMci, global_lb_name: str):
    return k_mci.os.run("{scripts_dir}/gclb-ingress.sh delete {cache_dir} "
...
def remove_gclb_cluster(k_mci: KubeMci, cluster_name: str, global_lb_name: str):
    ...
    return k_mci.os.run("{scripts_dir}/gclb-ingress.sh remove-clusters {cache_dir} "
...
def _sync_clusters_with_gclb(k_mci: KubeMci, gclb_ip: str, global_lb_name: str, cluster_names: List[str]):
    ...
    def _sync_configs(final_configs):
        ...
        return k_mci.os.run("KUBECONFIG={configs} {cache_dir}/kubectl "
                            "config view --flatten > {cache_dir}/clusters.yaml"
                            .format(configs=':'.join(final_configs), cache_dir=k_mci.config.cache_dir)) \
               | E.then | (
                   lambda _: k_mci.os.run("{scripts_dir}/gclb-ingress.sh create {cache_dir} "
                                          "{cache_dir}/clusters.yaml {global_lb_name} {project}"
...
def _create_gclb_ip(k_mci: KubeMci, global_lb_name: str):
    ...
    def _get_global_lb_ip(lb_name: str):
        return k_mci.os.run("{cache_dir}/google-cloud-sdk/bin/gcloud compute addresses describe --global {global_lb_name}"
    ...
    if gclb_ip is None:
        ...
        return k_mci.os.run("{cache_dir}/google-cloud-sdk/bin/gcloud compute addresses create --global {global_lb_name}"
...
def _setup(k_mci: KubeMci):
    return g.configure_auth(k_mci.gcloud) \
           | E.then | (lambda _: _install_kubemci(k_mci))
...
def _install_kubemci(k_mci: KubeMci):
    ...
    cur_os = 'darwin' if k_mci.os.is_macos() else 'linux'
    url = ("https://storage.googleapis.com/kubemci-release/release/v0.4.0/bin/{os}/amd64/kubemci".format(os=cur_os))
    k_mci.os.run("cd {cache_dir} && curl -L {url} > {cache_dir}/kubemci && chmod u+x {cache_dir}/kubemci"
