...
def test_can_generate_cron_job_template():
    def _run_dsl(args):
        ...
        assert result[0][0].templates[0]['kind'] == 'CronJob'
        assert result[0][0].templates[0]['metadata']['name'] == job_name
        container = result[0][0].templates[0]['spec']['jobTemplate']['spec']['template']['spec']['containers'][0]
