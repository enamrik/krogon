...
class MockOsSystem:
    ...
    def mock_kubectl_apply_temp_file(self, cluster_name: str, return_value: E.Either[Any, Any]):
        return self.mock_kubectl(cluster_name, command='apply -f /temp/template', return_value=return_value)
    ...
    def mock_clusters_list(self, cluster_names: List[str]):
        ...
        expectation = Setup(args=[cmd, MockSetup.any()], return_values=[E.Success('\n'.join(cluster_names))])
        MockSetup.mock(self.os_system.run, [expectation])
        return expectation
    ...
    def mock_install_vaultingkube(self, cluster_name: str, vault_address: str,
        ...
        cmd = '{script_dir}/install-vaultingkube.sh {cwd}/{cache_dir_name} ' \
            ...
            .format(cache_dir_name=Config.cache_folder_name(),
    ...
    def mock_create_gclb_address(self, gclb_name: str, return_value: E.Either[Any, Any]):
        cmd = '{cwd}/{cache_dir_name}/google-cloud-sdk/bin/gcloud compute addresses create --global {gclb_name}' \
            .format(cache_dir_name=Config.cache_folder_name(), cwd=MockFileSystem.cwd(), gclb_name=gclb_name)
        ...
        expectation = Setup(
            args=[
                '{cwd}/{cache_dir}/google-cloud-sdk/bin/gcloud compute addresses describe --global {gclb_name}'
                    .format(cache_dir=Config.cache_folder_name(),
