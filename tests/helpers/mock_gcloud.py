...
class MockGCloud:
    def __init__(self):
        ...
        PyMock.mock(self.init_client, return_values=[E.success(self.client)])
    ...
    def _mock_path_chain(mock, path_prefix, path: str, args, kwargs, exec_returns: List[Any]):
        ...
        if '.' not in path:
            ...
            PyMock.mock(gcloud_method, kwargs=kwargs, args=args, return_values=[gcloud_method_i])

            gcloud_method_i.execute = Mock(name=name+'-i.execute')
            PyMock.mock(gcloud_method_i.execute, return_values=exec_returns)
